...
export default async function MerchantReceiptsPage() {
  const { merchant } = await requireActiveMerchant();
  const { receipts, total } = await listReceipts({ merchantId: merchant.id, pageSize: 100 });

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-light tracking-tight text-foreground">Receipts</h1>
        <p className="text-sm text-muted-foreground">{total.toLocaleString()} receipts issued to your clients</p>
      </div>
          ...
          {receipts.length === 0 && (
            <TableRow>
              <TableCell colSpan={5} className="py-8 text-center text-muted-foreground">
                No receipts issued yet. They're generated automatically when a payment succeeds.
              </TableCell>
            </TableRow>
          )}
          {receipts.map((r) => (
